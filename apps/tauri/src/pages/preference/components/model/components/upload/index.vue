...
watch(selectPaths, async (paths) => {
	for await (const fromPath of paths) {
		try {
			...
			toast.success(t("pages.preference.model.hints.importSuccess"));
		} catch (error) {
...
<template>
	<div
		ref="drop"
		class="w-full flex flex-col cursor-pointer items-center justify-center gap-4 b b-color-1 rounded-lg b-dashed bg-color-8 transition hover:border-ant-primary"
		:class="{ 'border-ant-primary': dragenter }"
		@click="handleUpload"
	>
		<div class="i-solar:upload-square-outline text-12 text-ant-primary" />
