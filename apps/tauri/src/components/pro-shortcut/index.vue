...
<template>
	<ProListItem v-bind="props">
		<div
			ref="shortcutInput"
			align="center"
			class="relative h-8 min-w-32 flex cursor-text items-center justify-center b b-color-1 hover:b-primary-5 rounded-md b-solid px-2.5 text-color-3 outline-none transition focus:(b-ant-primary ring-2 ring-primary/20)"
			justify="center"
			:tabindex="0"
			@blur="handleBlur"
			@focus="handleFocus"
		...
		>
			<span v-if="pressedKeys.length === 0">
				{{
					isFocusing
						? $t("components.proShortcut.hints.pressRecordShortcut")
						: $t("components.proShortcut.hints.clickRecordShortcut")
				}}
			</span>
			...
			<div
				class="i-iconamoon:close-circle-1 absolute right-2 cursor-pointer text-4 transition hover:text-ant-primary"
